<!-- TopNavBar -->
<header class="sticky top-0 z-50 flex justify-between items-center w-full px-[var(--spacing-gutter)] max-w-[var(--spacing-max-width-content)] mx-auto bg-surface-container-lowest border-b border-outline-variant shadow-sm h-20">
    <div class="flex items-center gap-[var(--spacing-xs)]">
        <a href="{{ route('home') }}" class="font-headline-md text-headline-md font-bold text-primary">Hitech School SPMB</a>
    </div>
    <nav class="hidden md:flex items-center gap-[var(--spacing-lg)]">
    ...
    <div class="flex items-center gap-[var(--spacing-sm)]">
        ...
        @else
            ...
            @if (Route::has('register'))
                <a href="{{ route('register') }}" class="bg-primary text-on-primary px-[var(--spacing-md)] py-[var(--spacing-xs)] rounded-lg font-label-md text-label-md transition-all duration-200 active:scale-95 hover:bg-primary-container">Register Now</a>
            @endif
        @endauth
    </div>
</header>
